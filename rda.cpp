...
/* sel_stmt
 Parses strings in the language generated by the rule:
 <sel_stmt> -->  `TREVOR_SELECTION``(`<expr>`)` <stmt> 
 [ `TREVOR_ALT_SELECTION` <stmt> ]
 */
void sel_stmt(void) {
	printf("Enter <sel_stmt>\n");
	if (nextToken != TREVOR_SELECTION)
		syntaxError();
	else {
		lex();
		if (nextToken != LEFT_PAREN)
			...
			if (nextToken != RIGHT_PAREN)
				...
				lex();
				stmt();
				if (nextToken == TREVOR_ALT_SELECTION) {
					lex();
					stmt();
				}
			}
...
void term(void)
{
	...
	while (nextToken == ADD_OP || nextToken == MULT_OP || nextToken == DIV_OP || nextToken == MOD_OP)
	{
		lex();
		factor();
	}
